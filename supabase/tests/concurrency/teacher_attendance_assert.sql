...
begin
  ...
  if (select revision from public.attendance_records
      where session_id = '1d000000-0000-4000-8000-000000000019'
        and student_id = '15000000-0000-4000-8000-000000000001') <> 1 then
    raise exception 'teacher attendance absent-row race did not create revision 1';
  end if;
  ...
  if (select count(*) from public.notifications) <>
       (select notification_count from public.__test_teacher_attendance_side_effect_baseline where race = 'teacher-attendance')
     or (select count(*) from public.notification_outbox) <>
       (select outbox_count from public.__test_teacher_attendance_side_effect_baseline where race = 'teacher-attendance') then
    raise exception 'teacher attendance absent-row race created an unexpected notification outbox side effect';
  end if;
end
$$;
