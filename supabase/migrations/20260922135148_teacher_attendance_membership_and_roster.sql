...
begin
  ...
  limit 1;
  if actor_role IS DISTINCT FROM 'teacher' then raise exception 'teacher role required'; end if;
...
create or replace function public.get_teacher_attendance_roster(target_session_id uuid)
returns table (
  student_id uuid,
  display_name text,
  school_name text,
  attendance_status text,
  attendance_revision bigint
)
language sql
stable
security definer
...
as $$
  ...
  join public.cohort_students cs
   ...
   and cs.status = 'active'
   -- The derived flag also requires the cohort itself to be active.
  ...
  where auth.uid() is not null
    and public.is_teacher_for_session(ls.id)
    and ls.id = target_session_id
...
revoke all on function public.get_teacher_attendance_roster(uuid) from public;
revoke all on function public.get_teacher_attendance_roster(uuid) from anon;
revoke all on function public.get_teacher_attendance_roster(uuid) from authenticated;
revoke all on function public.get_teacher_attendance_roster(uuid) from service_role;
grant execute on function public.get_teacher_attendance_roster(uuid) to authenticated;
