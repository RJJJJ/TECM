...
3. server action 以 service role 在伺服器呼叫 Supabase Auth invitation，redirect 指向 `tecm://auth/callback`。
4. action 驗證 Auth user 沒有連到不同 organization/parent，再 idempotently 寫入 `parent_profiles.user_id` 及 invitation audit。
...
- Auth 建立成功但 DB link 失敗時保留 failed/retryable audit；重試會尋找相同 Auth user，不再建立新 user。
- DB link 成功但 Email delivery 失敗時保持可重發狀態。
