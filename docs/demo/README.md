# TECM AI Operations Demo Package

TECM 是一個面向澳門教育中心的 parent-facing iOS app + Admin Web + Supabase backend + n8n-ready AI operations system。這套 demo package 用於展示、驗收、重現與交付目前已完成的 booking、staff operations、AI follow-up 與安全自動化流程。

## Demo scope

- Parent booking
- Admin booking management
- AI follow-up suggestions
- Manual WeChat / WhatsApp follow-up
- Parent notification after booking confirmed
- n8n manual workflow
- n8n daily digest

## What this demo proves

- Full-stack app structure
- Supabase data model
- Admin operations workflow
- AI-assisted staff follow-up
- Safe automation design
- No unsafe WeChat auto-send
- Human-in-the-loop AI workflow

## Demo package files

- [5-minute demo script](demo-script-5min.md)
- [10-minute demo script](demo-script-10min.md)
- [Screenshot checklist](screenshot-checklist.md)
- [UAT checklist](uat-checklist.md)
- [Local runbook](local-runbook.md)
- [Staging readiness checklist](staging-readiness-checklist.md)
- [Architecture overview](architecture-overview.md)
- [Portfolio writeup](portfolio-writeup.md)
- [Known limitations and future work](known-limitations-and-future-work.md)

## Related n8n docs

- [n8n workflow templates README](../n8n/README.md)
- [Manual booking follow-up workflow setup](../n8n/manual-follow-up-workflow-setup.md)
- [Daily digest workflow setup](../n8n/daily-digest-workflow-setup.md)
- [n8n validation checklist](../n8n/n8n-validation-checklist.md)

## Final evidence pack

- [Evidence index](evidence-index.md)
- [UAT results](uat-results.md)
- [n8n validation results](n8n-validation-results.md)
- [Final demo run record](final-demo-run-record.md)
- [Local release candidate summary](release-candidate-summary.md)
- [Screenshots README](screenshots/README.md)
