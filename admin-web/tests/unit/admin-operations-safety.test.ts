...
test('credentialed Admin E2E is loopback-only and release results cannot silently skip', () => {
  ...
  const educationSpec = source('tests/e2e/education-operations.spec.ts');
  assert.match(educationSpec, /execFileSync\(\s*process\.execPath/);
  assert.match(educationSpec, /scripts\/test-run-identity\.mjs/);
  assert.match(educationSpec, /canonicalRunId/);
  assert.doesNotMatch(educationSpec, /TEST_ADMIN_UX_\$\{Date\.now\(\)\}/);
  ...
  assert.match(notificationSpec, /canonicalRunId/);
  assert.match(notificationSpec, /target_before/);
  assert.match(notificationSpec, /target_after/);
  assert.match(notificationSpec, /control_before/);
