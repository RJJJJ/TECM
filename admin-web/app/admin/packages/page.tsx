...
export default async function PackagesPage() {
  ...
  const feePlans = await supabase.from('fee_plans').select('id,name,credit_units,amount_minor,currency_code,is_active').eq('organization_id', organizationId).order('name');
  return <>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2">
      <Panel title="學生套票">{packages.error ? <ErrorState error={packages.error} fallback="讀取學生套票失敗，請稍後再試。" /> : !packages.data?.length ? <EmptyState>尚未有學生套票。</EmptyState> : <DataTable headers={['學生', '套票', '剩餘', '到期', '狀態']}>
        {packages.data.map((item: any) => <tr key={item.id}><td className="px-4 py-3 font-medium">{item.students?.display_name || '—'}</td><td className="px-4 py-3">{item.fee_plans?.name || '—'}</td><td className="px-4 py-3">{(item.credit_ledger ?? []).reduce((sum: number, row: any) => sum + Number(row.delta_units), 0)}</td><td className="px-4 py-3">{item.expires_on || '—'}</td><td className="px-4 py-3"><Badge tone={item.status === 'active' ? 'green' : 'slate'}>{item.status}</Badge></td></tr>)}
      </DataTable>}</Panel>
      <Panel title="套票方案">{feePlans.error ? <ErrorState error={feePlans.error} fallback="讀取套票方案失敗，請稍後再試。" /> : !feePlans.data?.length ? <EmptyState>尚未有套票方案。</EmptyState> : <DataTable headers={['名稱', '堂數', '金額', '狀態']}>
        {feePlans.data.map((plan) => <tr key={plan.id}><td className="px-4 py-3 font-medium">{plan.name}</td><td className="px-4 py-3">{plan.credit_units}</td><td className="px-4 py-3">{plan.currency_code} {(Number(plan.amount_minor) / 100).toFixed(2)}</td><td className="px-4 py-3"><Badge tone={plan.is_active ? 'green' : 'slate'}>{plan.is_active ? 'active' : 'inactive'}</Badge></td></tr>)}
      </DataTable>}</Panel>
