  ...
  return <>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2">
      ...
      <Panel title="套票方案">{feePlans.error ? <ErrorState error={feePlans.error} fallback="讀取套票方案失敗，請稍後再試。" /> : !feePlans.data?.length ? <EmptyState>尚未有套票方案。</EmptyState> : <DataTable headers={['名稱', '堂數', '金額', '狀態']}>
        {feePlans.data.map((plan) => <tr key={plan.id}><td className="px-4 py-3 font-medium">{plan.name}</td><td className="px-4 py-3">{plan.credit_units}</td><td className="px-4 py-3">{plan.currency_code} {(Number(plan.amount_minor) / 100).toFixed(2)}</td><td className="px-4 py-3"><Badge tone={plan.is_active ? 'green' : 'slate'}>{statusLabel(plan.is_active ? 'active' : 'inactive')}</Badge></td></tr>)}
      </DataTable>}</Panel>
