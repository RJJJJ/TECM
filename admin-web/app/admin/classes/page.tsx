...
export default async function ClassesPage() {
  ...
  return <>
    ...
    {error ? <ErrorState error={error} fallback="讀取班別失敗，請稍後再試。" /> : !data?.length ? <EmptyState action={<Link href="/admin/exam-cohorts" className="font-medium text-teal-700 underline">建立第一個班別</Link>}>尚未建立班別。</EmptyState> : <DataTable headers={['班別', '課程／程度', '導師', '人數', '狀態']}>
      {data.map((item: any) => <tr key={item.id}><td className="px-4 py-3 font-medium">{item.name}</td><td className="px-4 py-3">{item.courses?.title || item.subject}／{item.level}</td><td className="px-4 py-3">{item.teacher_profiles?.display_name || '—'}</td><td className="px-4 py-3">{item.cohort_students?.length ?? 0}</td><td className="px-4 py-3"><Badge tone={item.status === 'active' ? 'green' : 'slate'}>{statusLabel(item.status)}</Badge></td></tr>)}
    </DataTable>}
  </>;
}
