...
type ExamCohortRow = { id: string; name: string; subject: string; level: string; exam_date: string; weekday_pattern: string; status: string; course_id: string | null; teacher_profiles: { display_name: string | null } | null; courses: { title: string; category: string | null; level: string | null } | null; cohort_students: Array<{ id: string }> };
...
export default async function ExamCohortListPage({ searchParams }: { searchParams?: Promise<SearchParams> }) {
  ...
  let query = supabase.from('exam_cohorts').select('id,name,subject,level,exam_date,weekday_pattern,status,course_id,teacher_profiles(display_name),courses(title,category,level),cohort_students(id)').eq('organization_id', organizationId).eq('cohort_students.status', 'active').order('exam_date', { ascending: true });
  ...
  const [{ data, error }, { data: teachers, error: teacherError }, { data: courses, error: courseError }, { data: campuses, error: campusError }] = await Promise.all([
    query,
    ...
    supabase.from('campuses').select('id,name').eq('organization_id', organizationId).eq('is_active', true).order('name')
  ]);
  ...
  return <>
    ...
    {['admin', 'staff'].includes(role) ? <Panel title="建立班別"><CohortCreateForm teachers={(teachers ?? []).map(teacher => ({ id: teacher.id, label: teacher.display_name }))} courses={(courses ?? []).map(course => ({ id: course.id, title: course.title, category: course.category, level: course.level }))} campuses={(campuses ?? []).map(campus => ({ id: campus.id, label: campus.name }))} /></Panel> : null}
    {teacherError || courseError || campusError ? <div className="mt-5"><ErrorState error={teacherError || courseError || campusError} fallback="讀取建立班別所需資料失敗，請稍後再試。" /></div> : null}
    <section className="mt-5 space-y-5 rounded-xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
      ...
      {error ? <ErrorState error={error} fallback="讀取班別失敗，請稍後再試。" /> : cohorts.length === 0 ? <EmptyState>尚未有符合條件的班別，請先建立班別。</EmptyState> : <DataTable headers={['所屬課程', '班別名稱', '導師', '在讀學生', '狀態', '操作']}>
        {cohorts.map(cohort => <tr key={cohort.id}><td className="px-4 py-3">{cohort.courses ? <><p className="font-medium text-slate-900">{cohort.courses.title}</p><p className="text-xs text-slate-500">{cohort.courses.category ?? '-'}／{cohort.courses.level ?? '-'}</p></> : <span className="inline-flex rounded-lg bg-amber-100 px-2 py-1 text-xs font-semibold text-amber-900">尚未連結課程，請開啟班別修復</span>}</td><td className="px-4 py-3"><p className="font-medium text-slate-900">{cohort.name}</p><p className="text-xs text-slate-500">{cohort.exam_date} · {cohort.weekday_pattern === 'saturday' ? '星期六' : '星期日'}</p></td><td className="px-4 py-3">{cohort.teacher_profiles?.display_name ?? '待分配'}</td><td className="px-4 py-3">{cohort.cohort_students?.length ?? 0}</td><td className="px-4 py-3"><Badge tone={cohort.status === 'active' ? 'green' : cohort.status === 'cancelled' ? 'rose' : cohort.status === 'completed' ? 'slate' : 'amber'}>{statusLabel(cohort.status)}</Badge></td><td className="px-4 py-3"><Link className="text-sm font-medium text-teal-700 underline" href={`/admin/exam-cohorts/${cohort.id}`}>開啟班別</Link></td></tr>)}
      </DataTable>}
    </section>
  </>;
}
