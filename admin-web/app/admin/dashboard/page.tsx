...
export default async function DashboardPage() {
  ...
  const [sessions, attendance, leaves, makeups, ledgers, charges, payments, bookings] = await Promise.all([
  ...
  const results = [sessions, attendance, leaves, makeups, ledgers, charges, payments, bookings];
  ...
  return <>
    ...
    <div className="mt-6 grid gap-5 lg:grid-cols-2">
      ...
      <Panel title="系統狀態"><EmptyState>{results.some(result => result.error) ? '部分統計暫時未能讀取，請檢查 migration、RLS 及登入權限。' : '資料已同步至最新狀態。'}</EmptyState></Panel>
    </div>
  </>;
}
